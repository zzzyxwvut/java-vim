...
setlocal define=^\\C\\s*\\%(\\%(public\\\|protected\\\|private\\\|static\\\|final\\\|synchronized\\\|abstract\\\|native\\\|default\\\|strictfp\\)\\s\\+\\)\\?\\%(\\%(class\\\|@\?interface\\\|enum\\\|record\\)\\>\\\|\\%(\\%([[:upper:][:lower:]_][[:upper:][:lower:]0-9_<>\\[\\]]*\\)\\+\\s\\+\\ze\\i\\+\\s*(\\)\\)
