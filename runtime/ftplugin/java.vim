...
setlocal define=^\\C\\s*\\%(\\%(public\\\|protected\\\|private\\\|static\\\|final\\\|synchronized\\\|abstract\\\|native\\\|default\\\|strictfp\\)\\s\\+\\)*\\%(\\%(class\\\|@\?interface\\\|enum\\\|record\\)\\>\\\|\\%(\\%([[:keyword:]]<>\\[\\]]*\\)\\+\\s\\+\\ze\\i\\+\\s*(\\)\\)
